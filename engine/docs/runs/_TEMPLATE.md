...
- **Instrument calibration precondition (added 2026-07-16, harness-split incident)** — a strength instrument may back a decision only if, since its last code change, it (a) reproduces production color physics (`cc champs` auto-checks: audit `white_share` within ±20pts of the fleet's league share, else it prints a HARNESS CALIBRATION FAILURE and stamps `white_share_alert` in the jsonl row), and (b) if the harness itself is new or changed, matches a tournament-mode (gate-harness) same-pair match within noise. Two instruments disagreeing is an alarm to investigate, never a license to cite the convenient one.
