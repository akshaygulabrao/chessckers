...
| 16 | 2026-07-06 | official full start (same as 14/15) | v5 c64/b6 ~630K (warm from run 13 — same seed) | Adam lr=1e-3; **policy target = visits** (classic AZ), **value = pure z** (q-ratio 0) | **active** — strict pre-Gumbel CONTROL (Gumbel code physically absent; branches `ctl/pre-gumbel-run16`): fills the (visits, q=0) cell to test "did the Gumbel port introduce a bug" — does the freeze/slide recur without it, and at what rate? | [run16.md](runs/run16.md) |
