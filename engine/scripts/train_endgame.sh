...
#   - value/backup length discount γ=0.98 (gentle faster-win nudge)
#   - device split: self-play on CPU, training (SGD) on the accelerator
#     (--device cpu --train-device auto → MPS here; ~13-31× faster training)
#
...
exec .venv/bin/python -m chessckers_engine.selfplay_az_loop \
    --iterations 30 --games-per-iter 200 --sims 400 --epochs 3 --train-batch-size 256 \
