...
3.  **Demoted Kings — forced bottom-first.** A charge of distance $d$ demotes the **bottom $d$ Kings** (positions 1..$d$, 1-indexed from the bottom); there is **no player choice**. A demoted King becomes a Stone with `hasMoved = true`. Demoting from the bottom preserves the upper Kings, so a tower with more than $d$ Kings stays King-top. (Added v6; earlier versions let the player choose which Kings to demote, which produced a $\binom{n}{d}$ fan-out of legal charges — removed to shrink the branching factor.)
